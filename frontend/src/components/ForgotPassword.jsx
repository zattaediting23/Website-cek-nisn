...
const ForgotPassword = () => {
  ...
  const handleReset = async (e) => {
    ...
    try {
      ...
      // Navigate to login after 15 seconds so they can read the password
      setTimeout(() => {
        navigate('/login');
      }, 15000);

    } catch (err) {
      setError(err.response?.data?.error || 'Terjadi kesalahan saat memverifikasi data.');
    } finally {
      setLoading(false);
    }
  };
    ...
    <div className="flex items-center justify-center min-h-screen bg-gradient-to-br from-blue-500 to-indigo-700 p-4">
      <div className="w-full max-w-md bg-white/10 backdrop-blur-lg border border-white/20 rounded-3xl shadow-2xl p-8 overflow-hidden relative">
        ...
        <div className="relative z-10">
          <Link to="/login" className="inline-flex items-center text-blue-200 hover:text-white transition-colors mb-6 text-sm">
            <ArrowLeft className="w-4 h-4 mr-1" />
            Kembali ke Login
          </Link>

          <h2 className="text-3xl font-bold text-center text-white mb-2">Lupa Password</h2>
          <p className="text-center text-blue-100 mb-8 font-light text-sm">
            Masukkan NISN dan Tanggal Lahir Anda untuk mereset password ke pengaturan default.
          </p>
          ...
          {success && (
            <div className="bg-emerald-500/20 border border-emerald-500/50 text-emerald-100 px-4 py-4 rounded-xl mb-6 text-sm flex flex-col gap-3">
              <div className="flex items-center gap-2">
                <CheckCircle2 className="w-5 h-5 text-emerald-400" />
                <span className="font-semibold text-lg">Berhasil!</span>
              </div>
              <p>{success}</p>
              
              {newPassword && (
                <div className="bg-white/10 p-3 rounded-lg border border-emerald-400/30 text-center mt-2">
                  <p className="text-emerald-200 text-xs mb-1">Password Baru Anda:</p>
                  <p className="text-2xl font-mono font-bold text-white tracking-widest">{newPassword}</p>
                </div>
              )}
              
              <p className="text-xs opacity-75 mt-2 text-center">Mengarahkan ke halaman login...</p>
            </div>
          )}

          <form onSubmit={handleReset} className="space-y-5" style={{ display: success ? 'none' : 'block' }}>
            ...
            <div className="space-y-1">
              ...
              <div className="relative">
                ...
                <input
                  ...
                  className="w-full pl-11 pr-4 py-3 bg-white/10 border border-white/20 rounded-xl text-white placeholder-blue-300/50 focus:outline-none focus:ring-2 focus:ring-white/50 focus:bg-white/20 transition-all duration-300 [color-scheme:dark]"
                  required
                />
              </div>
            </div>
